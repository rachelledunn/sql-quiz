...
SELECT salespeople.givenname, salespeople.surname, SUM(orders.order_total), (SUM(orders.order_total) * 0.15) FROM salespeople LEFT JOIN orders ON (salespeople.id = orders.salesperson_id) GROUP BY orders.salesperson_id;